...
class BasicLayer(nn.Module):
    ...
    Args:
        ...
        这里的downsample和patchmerging之间的意思不同，最后还需观察一下
    """
...
class Nuclei_swin(nn.Module):
    ...
    def __init__(self,
        ...
        self.num_features = int(embed_dim * 2 ** (self.num_layers - 1))  # **是幂运算，这里指的是最后输出的特征维数，不建议变
        self.mlp_ratio = mlp_ratio
        num_patches = 50176
        ...
        # build layers
        self.layers = nn.ModuleList() #这样子model就可以被主程序识别
        ...
        head.append(nn.Linear(self.num_features // 2, 1))
        self.head = nn.Sequential(*head)
        #这里只是一个简单的MLP，我们的可能需要改动这里
    ...
    def forward_features(self, x, nuclei_mask):
        B, W, H, C = x.shape
        x = x.view(B, W*H, C)
        nuclei_mask = nuclei_mask.view(B, W*H, 1)
        if self.ape:
            x = x + self.absolute_pos_embed
        x = self.pos_drop(x)
        for layer in self.layers:
            x, nuclei_mask = layer(x, nuclei_mask)
        x = self.norm(x)
    ...
    def forward(self, x, nuclei_mask):
        x = self.forward_features(x, nuclei_mask)
        x = self.head(x)
        return x